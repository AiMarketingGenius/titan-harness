...
[atlas-api /api/titan/tts]  (ElevenLabs premium → audio/mpeg stream)
  ↓ audio stream
[User browser]
...
All three endpoints exist and are production-live on atlas-api (port 8081 on beast VPS):
...
- **GET /api/titan/tts?voice=alex&text=…** — returns `audio/mpeg` stream. Rate-limited 30/min per IP + daily cost cap (`ELEVENLABS_DAILY_CAP_USD`, default $10 per Sunday runway commit 3eeeb73). HTTP 429 on cap hit. Default voice: `alex` (canonical ElevenLabs voice ID `DZifC2yzJiQrdYzF21KH`, "Solon OS clone").
...
- ElevenLabs Turbo v2.5: ~$0.30 / 1K chars → ~$0.08 / 250-char reply
