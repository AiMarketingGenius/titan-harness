...
(function() {
  ...
  waitForPlatform((platformInfo) => {
    console.log(`[AI Memory] Perplexity content script active. Thread: ${platformInfo.threadId}`);
  ...
  function observeConversation(platformInfo) {
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          ...
          const assistantMsgs = node.matches?.('.answer-text, [data-testid="answer"]')
            ? [node]
            : (node.querySelectorAll?.('.answer-text, [data-testid="answer"], .prose') || []);
    ...
    document.querySelectorAll('.answer-text, [data-testid="answer"]')
  ...
  function processMessage(element, platformInfo) {
    const content = element.querySelector('.prose .whitespace-pre-wrap, .markdown')?.innerText?.trim()
      || element.querySelector('.prose')?.innerText?.trim();
    ...
    console.log(`[AI Memory] Perplexity response captured. Exchange #${exchangeCounter}, ${content.length} chars`);
  }
