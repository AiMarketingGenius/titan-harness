# Titan Dispatch - AMG Model Sovereignty v5.0.2

**Target canonical path:** `/opt/amg-docs/megaprompts/MEGAPROMPT_AMG_MODEL_SOVEREIGNTY_v5_0_2_2026-04-27.md`
**Staged path:** `/Users/solonzafiropoulos1/titan-harness/docs/megaprompts/MEGAPROMPT_AMG_MODEL_SOVEREIGNTY_v5_0_2_2026-04-27.md`
**Owner:** Achilles, CT-0427-59
**Status:** Round-3 patch staged for Titan judge run. Build phases remain blocked until the cheap-judge floor clears.

## 0. Binding Changes From v5.0.1

v5.0.2 closes the four round-2 judge gaps recorded in commit `50ef374`:

- §7 now defines the P0 metered-cost bypass contract instead of implying one.
- Solon override policy is moved out of revision notes and into an operative section.
- §8 now includes a cost-gate rollback receipt schema, not just prose.
- §5 and §11 now define partial-migration recovery so a mixed fleet cannot drift silently.
...
v5.0.2 remains a model-sovereignty and cost-discipline megaprompt. It does not reopen v4.0.x paper-freeze inventory.
...
Without all four conditions above, the fleet cap still blocks the call. "Important," "time-sensitive," or "close to shipping" is not P0.
...
if projected > fleet_cap then
  ...
  return {1, "p0_reserved", projected, new_agent, new_p0}
end
...
| Provider/Runtime | Data Exposure | Training/DPA Risk | v5.0.2 Rule |
|---|---|---|---|
...
| Kimi API | Vendor API | Requires data-use review | Metered and gated. |
...
## 13. Round-2 Revision Closure

| Round-2 Gap | v5.0.2 Resolution |
|---|---|
...
5. No Perplexity, Grok, or Gemini formal judge calls remain in v5.0.2.
...
14. Titan round-2 judge script is patched away from Gemini before reuse.
...
|---|---:|---|
| Technical soundness | 9.2 | Remaining cost and judge contradictions are closed in operative sections. |
| Completeness | 9.2 | All four round-2 judge gaps are now explicitly addressed. |
| Edge cases | 9.1 | P0 bypass, mixed-state migration halt, and degraded cost-gate replay are concrete. |
| Risk identification | 9.3 | Override abuse, partial-cutover drift, and bypass leakage are now named. |
| Cost discipline | 9.4 | Normal cap, P0 override cap, and replay accounting are separated cleanly. |
