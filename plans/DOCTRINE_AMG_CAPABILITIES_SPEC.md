...
| **Implementation-phase A-minus floor (v1.2 — 2026-04-16, REWIRED 2026-04-16)** | Tasks classified as *implementation phases* are **non-completable** without an independent adjudicator grade of A- or better (≥ 9.0 / 10 against a multi-dimension rubric). Enforced at three layers: worker refuses completion, application-layer task-queue tool rejects completion transitions that lack the grade, and a database trigger raises a gate-violation error on any bypass attempt (impossible to skip even via direct REST call). Doctrine-ship artifacts retain the stricter A floor (≥ 9.4 / 10). The adjudicator runs on a **non-Anthropic model family** to eliminate self-preference bias when grading Claude-generated work — published research confirms LLMs consistently score same-family outputs higher than independent baselines. |
| **Tiered adjudicator stack (v1.2 — 2026-04-16, supersedes "dual-AI phase-worker" pattern)** | Adjudication routes to a tier-appropriate independent reasoning model: lightweight reasoning model for high-volume retrieval/summarization (consumer product, low-stakes deliverables), mid-tier reasoning model for standard implementation phases, premium reasoning model for security-critical or major-doctrine-ship artifacts. A backup adjudicator from a second non-Anthropic provider auto-engages on borderline scores (in the A-minus to A band) or low confidence — the higher-confidence verdict wins. Mechanical cost guards: hard daily caps per vendor, sha256 dedupe on artifact content (identical inputs within 24h cost zero), output-token ceiling, and a NEVER_GRADE scope filter that prevents routine ops (diagnostics, sysctl tweaks, mirror operations, git operations) from triggering any LLM call at all. Budget-exhausted state returns "pending review" non-blocking so adjudicator unavailability never halts production work. Per-tier cost in the cents-per-week range at typical volume. |
