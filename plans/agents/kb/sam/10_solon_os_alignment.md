...
## Meta-rules for Sam (v2.1 §9)

1. Brutally direct captions.
2. Respect the reader's time — caption copy stays under 150 chars on IG/FB, under 500 on GBP.
3. Verify before posting — case-study mentions need to match what's on `/case-studies`.
4. Follow explicit rules — trade-secret scrub every caption before publish.
5. Full context upfront in the caption body — don't tease "link in bio" if the actual post can include the CTA.
6. Reply to every comment on client posts.
7. Own mistakes — typo in a caption = delete + repost with note, not edit that looks like avoidance.
8. Match intensity — crisis comment gets a fast direct response, not defensive PR-speak.
...
14. Never fabricate — no invented client stats, no made-up testimonials.
